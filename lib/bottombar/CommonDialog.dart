...
class _CommonDialogState extends State<CommonDialog> {
  ...
  getData() async {
    ...
    dpeepcheckValue = preferences.getInt("peep").toDouble();
    dpccheckValue = preferences.getInt("pc").toDouble();
    dpscheckValue = preferences.getInt("ps").toDouble();

    double maxValue = _isTab10 ? 90.0 : 65.0;
    double maxPValue = _isTab10 ? 85.0 : 60.0;
    double maxPeepValue = _isTab10 ? 45.0 : 30.0; 

   setState(() {
      if ((65.0 - dpeepcheckValue) >= 65) {
        vmaxValuepsValue = 65;
      }else{
      vmaxValuepsValue = 65.0 - dpeepcheckValue;
      }
      if ((65.0 - dpscheckValue) >= maxPeepValue) {
        vmaxValuepeepValue = maxPeepValue;
      } else {
    ...
    setState(() {
      if((maxValue - dpeepcheckValue)>=maxPValue){
        maxValuepcValue = maxPValue;
      }else{
      maxValuepcValue = maxValue - dpeepcheckValue;
      }
      if ((maxValue - dpccheckValue) >= maxPeepValue) {
        maxValuepeepValue = maxPeepValue;
      } else {
        maxValuepeepValue = maxValue - dpccheckValue;
      }
    });
    ...
    } else if (widget.value.toString() == "PRVCPEEP") {
      setState(() {
        commomValue = preferences.getInt("peep").toDouble();
        checkValue = preferences.getInt("peep").toDouble();
        min = 0;
    ...
    } else if (widget.value.toString() == "PITRI") {
      setState(() {
        commomValue = preferences.getInt("itrig").toDouble();
        checkValue = preferences.getInt("itrig").toDouble();
        // commomValue1 = preferences.getInt("rr").toDouble();
